...
function setupWatchers(options, cache, requestParser) {
  ...
  const possibleSvelteConfigs = knownSvelteConfigNames.map((cfg) => import_path2.default.join(root, cfg));
...
function resolveViaPackageJsonSvelte(importee, importer) {
  if (importer && isBareImport(importee)) {
    const importeePkgFile = import_require_relative.default.resolve(`${importee}/package.json`, import_path3.default.dirname(importer));
    const importeePkg = JSON.parse(import_fs3.default.readFileSync(importeePkgFile, { encoding: "utf-8" }));
    if (importeePkg.svelte) {
      return import_path3.default.resolve(import_path3.default.dirname(importeePkgFile), importeePkg.svelte);
    }
  }
}
function isBareImport(importee) {
  if (!importee || importee[0] === "." || importee[0] === "\0" || import_path3.default.isAbsolute(importee)) {
    return false;
  }
